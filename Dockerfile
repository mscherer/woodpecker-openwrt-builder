...
RUN dnf install --nodocs --setopt=install_weak_deps=False -y make wget perl-lib perl-FindBin perl-File-Compare perl-File-Copy "perl(Thread::Queue)" tar zstd && dnf clean all && rm -Rf /var/log/dnf5.log /var/lib/dnf/ /var/cache/
