FROM quay.io/fedora/fedora-minimal:44@sha256:020f11a1fc416276d7ec5a6bf1b34bc60aff48aa86622a2fc9db979f24d5364f
#
# empty space for easier rebasing
#
LABEL org.opencontainers.image.source="https://github.com/mscherer/woodpecker-openwrt-builder"

RUN dnf install --nodocs --setopt=install_weak_deps=False -y 'perl(JSON::PP)' util-linux unzip python3 which file gzip bzip2 patch diffutils 'perl(IPC::Cmd)' make wget perl-lib perl-FindBin perl-File-Compare perl-File-Copy "perl(Thread::Queue)" tar zstd perl-Time-Piece openssh-clients gettext-envsubst cowsay banner figlet jq && dnf clean all && rm -Rf /var/log/dnf5.log /var/lib/dnf/ /var/cache/
